...
const bundled = await build({
    ...
    stdin: {
        contents: `
            ...
            export { ItemType } from './src/types.ts';
...
test('iron ingots compress into an Iron Block', () => {
    assert.match(
        source,
        /push\(\s*3,\s*\[(?:\s*ItemType\.IRON_INGOT\s*,?){9}\s*\],\s*ItemType\.IRON_BLOCK,\s*1\s*\)/s,
    );
});
...
test('an Iron Block decompresses into nine iron ingots', () => {
    assert.match(
        source,
        /push\(\s*2,\s*\[\s*ItemType\.IRON_BLOCK,\s*null,\s*null,\s*null\s*\],\s*ItemType\.IRON_INGOT,\s*9\s*\)/s,
    );
});
...
test('every wood family supports the established crafting set', () => {
    for (const family of WOOD_FAMILIES) {
        ...
        assert.deepEqual(
            ...
            { type: ItemType.CRAFTING_TABLE, count: 1 },
        ...
        assert.deepEqual(
            ...
            { type: ItemType.BED_ITEM, count: 1 },
            `${family.name} bed`,
        );
        assert.deepEqual(
            checkRecipe([
            ...
            { type: ItemType.WOOD_PICKAXE, count: 1 },
            `${family.name} pickaxe`,
        );
        assert.deepEqual(
            checkRecipe([
                family.planks, family.planks, null,
                family.planks, ItemType.STICK, null,
                null, ItemType.STICK, null,
            ], 3),
            { type: ItemType.WOOD_AXE, count: 1 },
            `${family.name} axe`,
        );
        assert.deepEqual(
            checkRecipe([
            ...
            { type: ItemType.WOOD_SHOVEL, count: 1 },
            `${family.name} shovel`,
        );
        assert.deepEqual(
            checkRecipe([
            ...
            { type: ItemType.WOOD_SWORD, count: 1 },
            `${family.name} sword`,
        );
        assert.deepEqual(
            checkRecipe([
            ...
            { type: ItemType.WOOD_HOE, count: 1 },
            `${family.name} hoe`,
        );
        assert.deepEqual(
            checkRecipe([
...
test('wood recipes do not accept mixed plank species', () => {
    assert.equal(
        checkRecipe([
            ItemType.OAK_PLANKS, ItemType.SPRUCE_PLANKS,
            ItemType.OAK_PLANKS, ItemType.SPRUCE_PLANKS,
        ], 2),
        null,
    );
...
test('every stone variant crafts the complete stone tool set', () => {
    ...
    for (const material of STONE_TOOL_MATERIALS) {
        assert.deepEqual(
            ...
            { type: ItemType.STONE_PICKAXE, count: 1 },
        );
        assert.deepEqual(
            checkRecipe([material, material, null, material, S, null, null, S, null], 3),
            { type: ItemType.STONE_AXE, count: 1 },
        );
        assert.deepEqual(
