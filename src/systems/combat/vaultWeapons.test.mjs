...
test('interaction and held rendering use profiles, ammo, physical bolts, and distinct valid-use animation events', () => {
  ...
  assert.match(interaction, /(?:BlockType|ItemType)\.VAULT_BOLT/);
  ...
  assert.ok(interaction.indexOf('targetType === BlockType.CHEST') < interaction.indexOf('heldForUse?.type === ItemType.VAULT_CROSSBOW'));
  ...
  assert.match(runtime, /input\.heldItem !== (?:BlockType|ItemType)\.ECHO_TUNING_FORK/);
