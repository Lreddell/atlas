...
export class IndexedDbBackend implements StorageBackend {
    ...
    private decodeChunkRecord(value: unknown): ChunkStorageData | null {
        ...
        if (record.schema !== undefined) {
            ...
            return decodeStoredChunk(decodeChunkBody(record.body));
        }
        ...
        return decodeStoredChunk(record);
    }
    ...
    async writeChunks(worldId: string, chunks: ChunkBatchEntry[]): Promise<void> {
        ...
        for (const c of chunks) {
            const encoded = encodeStoredChunk(c);
            const data = { schema: 2, body: encodeChunkBody(encoded.blocks, encoded.light, encoded.meta, c.timestamp ?? Date.now(), encoded) };
