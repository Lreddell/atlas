import { decodeStoredChunk, encodeStoredChunk } from './chunkTransport';
...
export abstract class RegionBackendBase implements StorageBackend {
        ...
        const result = await migrateWorlds<WorldMetadata>({
            ...
            migrateOne: async (world) => {
                ...
                for (let i = 0; i < chunks.length; i += MIGRATION_BATCH) {
                    await this.api.writeChunks(world.id, chunks.slice(i, i + MIGRATION_BATCH).map(encodeStoredChunk));
                }
    ...
    async readChunk(worldId: string, cx: number, cz: number): Promise<ChunkStorageData | null> {
        const chunk = (await this.onFs(worldId)) ? await this.api.readChunk(worldId, cx, cz) : await this.legacy.readChunk(worldId, cx, cz);
        return chunk ? decodeStoredChunk(chunk) : null;
    }
    ...
    async writeChunks(worldId: string, chunks: ChunkBatchEntry[]): Promise<void> {
        ...
        return (await this.onFs(worldId)) ? this.api.writeChunks(worldId, chunks.map(encodeStoredChunk)) : this.legacy.writeChunks(worldId, chunks);
    }
    ...
    async exportWorld(worldId: string): Promise<ExportedWorldData> {
        if (await this.onFs(worldId)) {
            ...
            return encodeExportedWorld(meta, (await this.api.readChunksAll(worldId)).map(decodeStoredChunk));
        }
    ...
    async importWorld(data: ExportedWorldData): Promise<WorldMetadata> {
        ...
        for (let i = 0; i < chunks.length; i += MIGRATION_BATCH) {
            await this.api.writeChunks(meta.id, chunks.slice(i, i + MIGRATION_BATCH).map(encodeStoredChunk));
        }
