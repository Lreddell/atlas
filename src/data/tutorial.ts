...
export const TUTORIAL_SECTIONS: TutorialSection[] = [
    ...
    },
    {
        ...
        paragraphs: [
            ...
            'The first two forms are shielded by tower crystals: break every crystal of that form to expose the core. The final crystal is optional; destroying it removes the Storm shard barrier and its overload volleys. Each lit tower carries the Warden\'s polarity on its climb faces: oppose it to cling and climb, and when the Warden swaps colour the tower swaps with it. A flux window opens before each flip: flip (R) inside it to hold on, or the settled tower throws you off. Press C on a wall for a magnetic launch back toward the platform (aim for the landing pools).',
            'Its first form duels you on the platform (Volley, Lash, a Draw into a Repel burst, and a Charge down a marked lane) while its one crystal stands. Its second, the Aegis, lights two towers and hovers out to contest whichever one you climb, alternating aimed shots with a marked sweep of one wall side. Each crystal powers one of these patterns; break both and it crashes into the pool below that tower, reeling. Its last form, the Storm, holds the final tower and flips polarity on a beat: the tower flips with every beat, each flip is a ring you must be pinned against, and you can attack immediately or climb the final tower to weaken its shard attacks.'
        ],
        bullets: [
            'White charged bolts can be struck back: aim at the boss and time a fresh weapon swing so its hit meets the bolt. Holding attack does not automatically return bolts.',
            'The aerial slam follows you until its landing circle turns white. Move out after the lock, then answer the polarity ring. Expect a delayed backswing after familiar melee attacks.',
